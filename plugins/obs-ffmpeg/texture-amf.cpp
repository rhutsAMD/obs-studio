...
#undef add_profile
	...
	if (amf_codec_type::AV1 == codec) {
		p = obs_properties_add_bool(props, "screen_content_tools", obs_module_text("AMF.ScreenContentTools"));
		p = obs_properties_add_bool(props, "palette_mode", obs_module_text("AMF.PaletteMode"));
	}
...
static bool amf_avc_init(void *data, obs_data_t *settings)
{
	...
	info("settings:\n"
	     ...
	     "\tpre-analysis: %s\n"
	     "\tparams:       %s",
	     rc_str, bitrate, qp, gop_size, preset, profile, bf, enc->cx, enc->cy, pa_enabled, ffmpeg_opts);
...
static bool amf_hevc_init(void *data, obs_data_t *settings)
{
	...
	info("settings:\n"
	     ...
	     "\tpre-analysis: %s\n"
	     "\tparams:       %s",
	     rc_str, bitrate, qp, gop_size, preset, profile, enc->cx, enc->cy, pa_enabled, ffmpeg_opts);
...
static bool amf_av1_init(void *data, obs_data_t *settings)
{
	...
	info("settings:\n"
	     ...
	     "\tpre-analysis:         %s\n"
	     "\tparams:               %s",
	     rc_str, bitrate, qp, gop_size, preset, profile, enc->cx, enc->cy, screen_content_tools_enabled,
	     palette_mode_enabled, pa_enabled, ffmpeg_opts);
